...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Opgave 2
            var resultTwo = planets.Where((obj) => obj.Name.ToLower().Contains("y"));

            // Opgave 3
            var resultThree = planets.Where((obj) => obj.Name.Length > 6 && obj.Name.Length < 15);

            // Opgave 4
            var resultFour = planets.Where((obj) => obj.Name[1] == 'a' && obj.Name[obj.Name.Length - 1] == 'e');

            // Opgave 5
            var resultFive = planets.Where((obj) => obj.RotationPeriod > 40)
            .OrderBy((obj) => obj.RotationPeriod);

            // Opgave 6
            var resultSix = planets.Where((obj) => obj.RotationPeriod > 10 && obj.RotationPeriod < 20)
                .OrderBy((obj) => obj.Name);

            // Opgave 7
            var resultSeven = planets.Where((obj) => obj.RotationPeriod > 30)
                .OrderBy((obj) => obj.Name)
                .ThenBy((obj) => obj.RotationPeriod);

            // Opgave 8
            var resultEight = planets.Where((obj) => (obj.RotationPeriod < 30 || obj.SurfaceWater > 50) && obj.Name.Contains("ba"))
                .OrderBy((obj) => obj.Name)
                .ThenBy((obj) => obj.SurfaceWater)
                .ThenBy((obj) => obj.RotationPeriod);

            // Opgave 9
            var resultNine = planets.Where((obj) => obj.SurfaceWater > 0)
                .OrderByDescending((obj) => obj.SurfaceWater);

            // opgave 10
            var resultTen = planets.Where((obj) => obj.Diameter != 0 && obj.Population != 0)
                .OrderBy((obj) => (4 * Math.PI * obj.Diameter) / obj.Population);

            // opgave 11
            var ellevenExceptList = new List<Planet>() { new Planet() { RotationPeriod = 0 } };
            var resultElleven = planets.Except(ellevenExceptList, new RotationComparer());

            // Opgave 12
            var twelveResultOne = planets.Where((obj) => obj.Name.StartsWith("a") || obj.Name[obj.Name.Length - 1] == 's');

            var twelveResultTwo = planets.Where((obj) => obj.Terrain != null)
                .SelectMany((obj) => obj.Terrain, (plan, terr) => new { plan, terr })
                .Where((obj) => obj.terr == "rainforests")
                .Select((obj) => obj.plan);

            // Opgave 13
            var thirteenResult = planets.Where((obj) => obj.Terrain != null)
                .SelectMany((obj) => obj.Terrain, (plan, terr) => new { plan, terr })
                .Where((obj) => obj.terr.Contains("desert"))
                .Select((obj) => obj.plan);

            // Opgave 14
            var tourteenResult = planets.Where((obj) => obj.Terrain != null)
                .SelectMany((obj) => obj.Terrain, (plan, terr) => new { plan, terr })
                .Where((obj) => obj.terr.Contains("swamp"))
                .Select((obj) => obj.plan)
                .OrderBy((obj) => obj.RotationPeriod)
                .ThenBy((obj) => obj.Name);

            // Opgave 15
            Regex fifteenRegex = new Regex(@"([aeiou])\1", RegexOptions.IgnoreCase);

            var fifteenResult = planets.Where((obj) => fifteenRegex.IsMatch(obj.Name) == true);

            // Opgave 16
            Regex sixteenRegex = new Regex(@"([klrn])\1", RegexOptions.IgnoreCase);

            var sixteenResult = planets.Where((obj) => sixteenRegex.IsMatch(obj.Name) == true)
                .OrderByDescending((obj) => obj.Name);
